...
async function send() {
  ...
  // If the backend return a list(message or url of websites)
  if (Array.isArray(reply)){
    reply.forEach(r => {
      addmsg(r.message, "aurora");
      if(r.url) window.open(r.url,"_blank"); //Opens any sites in new tab
      if(!Mute) speak(r.message);
    });
  } else {
    addmsg(reply, "aurora");
    if(!Mute) speak(reply);
